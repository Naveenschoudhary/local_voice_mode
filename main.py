...
class QwenMultimodalProcessor:
    ...
    def __init__(self):
        # Use MPS for Apple Silicon, CUDA for NVIDIA GPU, or CPU as fallback
        ...
        # Load model with appropriate device and dtype settings
    ...
    async def generate(self, text):
        ...
        async with self.lock:
            try:
                ...
                # Move generated ids back to CPU for decoding
                generated_ids = generated_ids.cpu()
                inputs_cpu = {k: v.cpu() if hasattr(v, 'to') else v for k, v in inputs.items()}
...
class KokoroTTSProcessor:
    ...
    def __init__(self):
        ...
        try:
            # Initialize Kokoro TTS pipeline with English and explicit repo_id
            self.pipeline = KPipeline(
                lang_code='a',
                repo_id='hexgrad/Kokoro-82M',  # Explicitly set repo_id to suppress warning
                # Suppress other warnings at runtime level
                ignore_warnings=True
            )
            
            # Set English voice to sarah
